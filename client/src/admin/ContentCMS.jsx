  ...
  const handleOpenCreate = () => {
    ...
    if (activeTab === 'blogs') {
      ...
      setStoryForm({
        ...
        quote: '',
        image: '',
        courseTitle: 'Full-Stack Cloud Engineering',
      });
    }
    setShowModal(true);
  };
  ...
  const handleOpenEdit = (item) => {
    ...
    if (activeTab === 'blogs') {
      ...
      setStoryForm({
        ...
        salaryHike: item.salaryHike || '+120%',
        quote: item.quote || '',
        image: item.image || '',
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (activeTab === 'blogs') {
        ...
        if (editingItem) {
          const res = await axios.put(`/api/content/success-stories/${editingItem._id}`, storyForm, { headers });
          if (res.data.success) setStories(stories.map(s => s._id === editingItem._id ? res.data.story : s));
        } else {
          const res = await axios.post('/api/content/success-stories', storyForm, { headers });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex rounded-xl bg-slate-900 border border-slate-800 p-1">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono transition-colors ${
                activeTab === 'blogs' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono transition-colors ${
                activeTab === 'faqs' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono transition-colors ${
                activeTab === 'stories' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
              }`}
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-xs shadow-lg shadow-cyan-500/20 transition-all"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {loading ? (
            ...
            blogs.map((b) => (
              <div
                key={b._id}
                className="group rounded-2xl border border-slate-800 bg-slate-900/60 p-5 hover:border-cyan-500/40 transition-all flex flex-col justify-between"
              >
                <div>
              ...
              <form onSubmit={handleSave} className="space-y-4 text-xs font-mono">
                ...
                {activeTab === 'stories' && (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-slate-400 uppercase mb-1">Salary Hike Metric</label>
                        <input
                          type="text"
                          value={storyForm.salaryHike}
                          onChange={(e) => setStoryForm({ ...storyForm, salaryHike: e.target.value })}
                          placeholder="+140%"
                          className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white font-sans focus:outline-none focus:border-cyan-500"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="text"
                          value={storyForm.courseTitle}
                          onChange={(e) => setStoryForm({ ...storyForm, courseTitle: e.target.value })}
                          className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white font-sans focus:outline-none focus:border-cyan-500"
                        />
                      </div>
                    </div>
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2 rounded-xl bg-cyan-500 text-slate-950 font-bold font-sans hover:bg-cyan-400 transition-colors"
                  >
