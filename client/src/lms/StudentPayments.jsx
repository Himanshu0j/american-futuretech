    ...
    <div className="p-6 sm:p-8 space-y-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#d8ffd2] border border-[#76ff8a]/40 text-[#1a361d] text-xs font-bold uppercase tracking-wider mb-2">
            <CreditCard className="w-3.5 h-3.5 text-[#2d5c36]" />
        ...
        <div className="flex items-center gap-3">
          <div className="px-5 py-3 rounded-2xl bg-white border border-slate-200 text-right shadow-xs">
            ...
            <div className="text-2xl font-heading font-black text-[#2d5c36] mt-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            ...
            <Receipt className="w-4 h-4 text-[#2d5c36]" />
          </div>
          <div className="text-2xl font-heading font-black text-[#1a361d]">{payments.length}</div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
          ...
          <div className="text-2xl font-heading font-black text-[#2d5c36]">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            ...
            <ShieldCheck className="w-4 h-4 text-[#9e4f8f]" />
          </div>
          <div className="text-sm font-bold text-[#1a361d] mt-1">Active Student • Good Standing</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center max-w-xl mx-auto shadow-xs">
          ...
          <h3 className="text-base font-heading font-bold text-[#1a361d] mb-1">No Invoices Found</h3>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-xs">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-[#fffff2] text-[11px] font-bold text-slate-600 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100 text-xs">
                {payments.map((p) => (
                  <tr key={p._id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-4 px-6 font-mono font-bold text-[#1a361d]">
                    ...
                    <td className="py-4 px-6">
                      <span className="inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-[#fffff2] text-slate-700 border border-slate-200">
                        {p.paymentType === 'DEPOSIT' ? '$99 Seat Deposit' : 'Full Tuition'}
                      </span>
                    </td>
                    <td className="py-4 px-6 font-mono font-bold text-[#2d5c36]">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${
                        ...
                        {p.status === 'Paid' && <CheckCircle2 className="w-3 h-3 text-[#2d5c36]" />}
                    ...
                    <td className="py-4 px-6 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white hover:bg-slate-100 border border-slate-300 text-[#1a361d] text-xs font-semibold shadow-xs transition-all"
                      >
      ...
      <AnimatePresence>
        {selectedInvoice && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            ...
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-[#fffff2]">
                <div className="flex items-center gap-2">
                  <Receipt className="w-4 h-4 text-[#2d5c36]" />
                  <h3 className="text-sm font-bold text-[#1a361d] font-mono">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#9e4f8f] hover:bg-[#582c50] text-white text-xs font-bold transition-colors shadow-xs"
                  >
              ...
              <div className="p-8 overflow-y-auto space-y-6 text-slate-700 print:text-black print:bg-white text-xs" id="printable-area">
                ...
                <div className="flex justify-between items-start border-b border-slate-200 pb-6">
                  <div>
                    <div className="text-lg font-heading font-black tracking-tight text-[#1a361d]">
                  ...
                  <div className="text-right">
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-[#d8ffd2] text-[#1a361d] uppercase">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div>
                    ...
                    <div className="text-[#1a361d] font-bold mt-1 text-sm">
                      {selectedInvoice.student?.name || 'Student Account'}
                    </div>
                    <div className="text-slate-600 text-xs">{selectedInvoice.student?.email || selectedInvoice.billingEmail}</div>
                  </div>
                  <div className="text-right">
                    <span className="text-slate-400 block uppercase tracking-wider text-[10px] font-bold">TRANSACTION ID:</span>
                    <div className="text-[#1a361d] font-mono font-bold mt-1 break-all text-xs">
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-[#fffff2] text-slate-600 uppercase font-bold text-[11px] border-b border-slate-200">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      <tr>
                        ...
                        <td className="py-3 px-4 text-right font-mono font-bold text-[#1a361d]">
                          ${selectedInvoice.amount} USD
                        </td>
                      </tr>
                    </tbody>
                    <tfoot className="bg-[#fffff2] font-mono border-t border-slate-200">
                      <tr>
                        <td colSpan={3} className="py-3 px-4 text-right font-bold text-slate-600">TOTAL:</td>
                        <td className="py-3 px-4 text-right font-bold text-[#2d5c36] text-sm">
                          ${selectedInvoice.amount} USD
                        </td>
                      </tr>
                    </tfoot>
